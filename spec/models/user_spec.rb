...
describe User, type: :model do
  ...
  describe 'Fields missing when registering (is_guest = false)' do
  ...
  describe 'Guest accounts should create as long as valid email is present' do
    it 'should fail to create if email is missing' do
  ...
  describe 'Emails' do
    it 'should fail if an email already exists' do
      ...
      user2 = User.create(name: 'Elizabeth Childs', given_name: 'Beth', email: 'abc@abc.com', password: 'abcdefg')
      expect(user2.persisted?).to be(false)
    end
    it 'should fail if an email already exists in a different case' do
      user = User.create(name: 'Abraham Lincoln', given_name: 'Abe', email: 'abc@abc.com', password: 'abcdefg')
      user2 = User.new(name: 'Elizabeth Childs', given_name: 'Beth', email: 'ABC@abc.com', password: 'abcdefg')
    ...
    it 'should fail if email is not entered in the right format' do
      user = User.create(name: 'Abraham Lincoln', given_name: 'Abe', email: 'this.is.an.email.notreally', password: 'abcdefg')
      expect(user.persisted?).to be(false)
    end
  end
  ...
  describe 'Passwords' do
    it 'should fail if passwords do not match' do
      user = User.create(name: 'Abraham Lincoln', given_name: 'Abe', email: 'abc@abc.com', password: 'abcdefg', password_confirmation: 'abcdef')
      expect(user.persisted?).to be(false)
    end
    it 'should fail if passwords are entered with incorrect cases' do
      user = User.create(name: 'Abraham Lincoln', given_name: 'Abe',  email: 'abc@abc.com', password: 'abcdefg', password_confirmation: 'Abcdefg')
      expect(user.persisted?).to be(false)
    end
    ...
    it 'should fail if password is shorter than 5 characters' do
      user = User.create(name: 'Abraham Lincoln', given_name: 'Abe', email: 'abc@abc.com', password: 'abcd', password_confirmation: 'abcd')
      expect(user.persisted?).to be(false)
    end
    it 'should pass if password is 5 or more characters' do
      user = User.create(name: 'Abraham Lincoln', given_name: 'Abe', email: 'abc@abc.com', password: 'abcde', password_confirmation: 'abcde')
      expect(user.persisted?).to be(true)
    end
  end
